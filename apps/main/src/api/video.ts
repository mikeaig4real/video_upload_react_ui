...
export async function getVideos(filter: VideoFilters) {
  const res = await api.get<APIResponse<UploadedVideo[]>>("/video", {
    params: filter,
  });
  return res.data;
}
...
export async function getVideoById(id: string) {
  const res = await api.get<APIResponse<UploadedVideo | null>>(`/video/${id}`);
  return res.data;
}
...
export async function createVideo(uploadedVideo: UploadedVideo) {
  try {
    ...
    const res = await api.put<APIResponse<UploadedVideo>>("/video", videoFile);
    return res.data;
  } catch (error) {
    log({
      error,
