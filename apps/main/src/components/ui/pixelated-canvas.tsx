  ...
  React.useEffect(() => {
    ...
    img.onload = () => {
      ...
      const animate = () => {
        ...
        const t = now * 0.001 * jitterSpeed; // animation time
        ...
        for (const s of samples) {
          ...
          if (distortionMode === "repel" || distortionMode === "attract") {
            ...
            const radius = distortionStrength;
            ...
            const angle = distortionStrength * 0.05;
            const cosA = Math.cos(angle);
            const sinA = Math.sin(angle);
            drawX = centerX + cosA * dx - sinA * dy;
            drawY = centerY + sinA * dx + cosA * dy;
          }

          // JITTER
          if (jitterStrength > 0) {
            const k = s.seed * 43758.5453;
            const jx = Math.sin(t + k) * jitterStrength;
            const jy = Math.cos(t + k * 1.13) * jitterStrength;
            drawX += jx;
            drawY += jy;
          }
